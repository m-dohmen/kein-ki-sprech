...
**Vorher/Nachher-Paare beisteuern.** `skills/kein-ki-sprech/references/beispiele.md` wächst mit jedem Muster. Ein neues Paar folgt dem bestehenden Format: `**Vorher:**` / `**Nachher:**`, kein erfundener Fakt ohne Kennzeichnung als Beispielwert (siehe Einleitung der Datei).
...
2. Änderung in `skills/kein-ki-sprech/` – an `SKILL.md` und/oder `references/beispiele.md`.
...
4. Grep-Gegenprobe: `grep -rc '—' skills/kein-ki-sprech/` muss außer der einen Typografie-Erwähnung in `SKILL.md` überall `0` liefern – der Skill verlangt den Halbgeviertstrich, nicht den Geviertstrich.
